...
def oura_sync(body: OuraSync, session: Session = Depends(get_session)):
    ...
    return sync_oura(session, token, days=body.days)
...
async def apple_health(file: UploadFile = File(...), session: Session = Depends(get_session)):
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
        ...
        while chunk := await file.read(1 << 20):  # stream in 1MB chunks
            tmp.write(chunk)
    try:
        return import_apple_health(session, tmp_path)
    finally:
